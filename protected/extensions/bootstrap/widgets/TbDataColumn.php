...
class TbDataColumn extends CDataColumn
{
    ...
    public function getStatus($model_id){
        $votingObj = Votings::model()->findByPk($model_id);
        $today = new DateTime(date("Y-m-d G:i:s"));
        $today->setTimezone(new DateTimeZone("Asia/Almaty"));

        $end_date_arr = explode("-", $votingObj->end_date);
        $start_date_arr = explode("-", $votingObj->start_date);

        $end_time_arr = explode(":", $votingObj->end_time);
        $start_time_arr = explode(":", $votingObj->start_time);

        $end_date = new DateTime();
        $end_date->setTimezone(new DateTimeZone(("Asia/Almaty")));
        $end_date->setDate($end_date_arr[0], $end_date_arr[1], $end_date_arr[2]);
        $end_date->setTime($end_time_arr[0], $end_time_arr[1], $end_time_arr[2]);

        $start_date = new DateTime();
        $start_date->setTimezone(new DateTimeZone(("Asia/Almaty")));
        $start_date->setDate($start_date_arr[0], $start_date_arr[1], $start_date_arr[2]);
        $start_date->setTime($start_time_arr[0], $start_time_arr[1], $start_time_arr[2]);
